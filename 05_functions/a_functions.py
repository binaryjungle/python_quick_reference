...
#Functions can take inputs. The inputs are just mentioned with some variable name.
#Function doesn't gives back (RETURN) anything.
...
#The inputs that we pass can be with a different variable name.
#Placeholders for all arguments should be given.
...
#Function that can take infinite or variable number of arguments.
#tuples are created using these varying number of arguments list
...
#2 values (keys and values) is sent and processed in one loop.
def max_bmi(**kwargs):
    bigvalue = 0
    Weight = "00"
    Height = 0.0
...
#Example to calculate bmi is mentioned below 
